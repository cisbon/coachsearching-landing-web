...
CREATE INDEX IF NOT EXISTS idx_coaches_approved ON public.coaches(approved);
CREATE INDEX IF NOT EXISTS idx_coaches_invite_code ON public.coaches(invite_code);
...
DROP TRIGGER IF EXISTS increment_code_usage ON public.coaches;
...
DROP TRIGGER IF EXISTS set_coach_approved_at ON public.coaches;
...
INSERT INTO public.invite_codes (code, created_by, is_active, max_uses) VALUES
...
-- View: pending_coaches
-- Shows all coaches waiting for approval
CREATE OR REPLACE VIEW public.pending_coaches AS
SELECT
    id,
    name,
    email,
...
GRANT INSERT ON public.coaches TO authenticated;
...
GRANT INSERT ON public.coaches TO anon;
...
--    - Go to Supabase Dashboard > Table Editor > coaches
...
--
-- 6. OPTIONAL: Set up email notifications when coaches are approved
--    - Use Supabase Edge Functions or webhooks
--    - Trigger on UPDATE of coaches table when approved = true
--
-- 7. OPTIONAL: Create an admin panel to manage approvals
--    - Build a separate admin interface
--    - Use service_role key (keep it secret!) for admin operations
--    - Query pending_coaches view for coaches awaiting approval
-- =====================================================
